...
export const ProductCarousel = ({
  ...
  const validVideos = media.videos.filter(video => video && video.trim() !== '').map(video => {
    // Converte URLs do YouTube para formato de embed
    if (video.includes('youtube.com') || video.includes('youtu.be')) {
      const videoId = video.includes('youtu.be') ? video.split('/').pop() : video.split('v=')[1]?.split('&')[0];
      ...
      <CarouselContent>
        {validPhotos.map((foto, index) => <CarouselItem key={`foto-${index}`}>
            <div className="relative aspect-[4/3] lg:aspect-[16/9]">
              <img src={foto || '/placeholder.svg'} alt={`${productName} - Foto ${index + 1}`} className="w-full h-full object-contain rounded-xl" />
            </div>
          </CarouselItem>)}
        {validVideos.map((video, index) => <CarouselItem key={`video-${index}`}>
            <div className="relative aspect-[4/3] lg:aspect-[16/9]">
              <iframe src={video} title={`${productName} - Vídeo ${index + 1}`} className="w-full h-full rounded-xl" allowFullScreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" loading="lazy" />
            </div>
          </CarouselItem>)}
      </CarouselContent>
